    ...
    datasets: [
      {
        label: "Movimientos",
        data: [
          movimientos.filter((m: any) => m.en_proceso && !m.aceptado && !m.cancelado).length,
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card className="bg-yellow-100 dark:bg-zinc-800 dark:text-white">
          <CardContent className="py-4">
            ...
            <p className="text-2xl font-bold text-yellow-900">{movimientosPendientes.length}</p>
          </CardContent>
        </Card>
        <Card className="bg-green-100 dark:bg-zinc-800 dark:text-white">
          <CardContent className="py-4">
            ...
            <p className="text-2xl font-bold text-green-900">{verificaciones.length}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow dark:bg-zinc-800 dark:text-white">
        ...
        <div className="w-full h-72">
          <Bar data={estadisticas} options={{ responsive: true, plugins: { legend: { display: false } } }} />
        </div>
      </div>
            ...
            <tbody>
              {ultimosMovimientos.map((mov: any) => (
                <tr key={mov.id_movimiento} className="">
                  ...
                  <td className="py-2 px-4">{new Date(mov.created_at).toLocaleDateString()}</td>
