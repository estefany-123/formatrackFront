import GraficaBase from "@/components/organismos/graficasBase/graficas";
...
const ProgramaEstadisticas = () => {
  ...
  if (!programas || programas.length === 0)
    return <p>no hay datos de programas de formación</p>;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-3">
      <GraficaBase
        ...
        tipo="bar"
        data={barData}
        titulo="programas por area"
      />
      <GraficaBase
        className="bg-white dark:bg-zinc-800 dark:text-white"
        tipo="pie"
        data={pieData}
        titulo="Distribución de estado"
      />
      <GraficaBase
        className="bg-white dark:bg-zinc-800 dark:text-white"
        tipo="line"
        data={lineData}
        titulo="Programas de formación  creados por mes"
      />
    </div>
  );
};
