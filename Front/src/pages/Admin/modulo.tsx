...
const ModulosTable = () => {
    ...
    const {modulos, isLoading, isError, error, addModulo, changeState } = useModulo();
    ...
    const ModulosWithKey = modulos?.filter(modulos => modulos?.id_modulo !== undefined).map((modulos) => ({
        ...modulos,
        key: modulos.id_modulo ? modulos.id_modulo.toString() : crypto.randomUUID(),
    ...
    return (
        ...
        <div className="p-4">
            ...
            <Buton text="Añadir Modulo" onPress={() => setIsOpen(true)} type="button" color="primary" variant="solid" className="mb-8" />

             <Modall ModalTitle="Agregar modulo" isOpen={isOpen} onOpenChange={handleClose}>

                <FormModulos id="user-form" addData={handleAddModulo} onClose={handleClose} />
                <button type="submit" form="user-form" className="bg-blue-500 text-white p-2 rounded-md">
                    Guardar
                </button>
            </Modall>

            <Modall ModalTitle="Editar Modulo" isOpen={IsOpenUpdate} onOpenChange={handleCloseUpdate}>
                {selectedModulo && (
                    <FormUpModulo modulos={ModulosWithKey ?? []} moduloId={selectedModulo.id_modulo} id="FormUpdate" onclose={handleCloseUpdate} />
                )}
