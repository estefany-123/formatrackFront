...
const RutasTable = () => {
    ...
    const rutasWithKey = rutas?.filter(rutas => rutas?.id_ruta !== undefined).map((rutas) => ({
        ...rutas,
    ...
    return (
        ...
        <div className="p-4">
            ...
            <Buton text="Añadir Ruta" onPress={() => setIsOpen(true)} type="button" color="primary" variant="solid" className="mb-8" />

             <Modall ModalTitle="Agregar Ruta" isOpen={isOpen} onOpenChange={handleClose}>

                <FormRutas id="user-form" addData={handleAddCentro} onClose={handleClose} />
                <button type="submit" form="user-form" className="bg-blue-500 text-white p-2 rounded-md">
                    Guardar
                </button>
            </Modall>

            <Modall ModalTitle="Editar Ruta" isOpen={IsOpenUpdate} onOpenChange={handleCloseUpdate}>
                {selectedRuta && (
                    <FormUpRutas rutas={rutasWithKey ?? []} rutaId={selectedRuta.id_ruta} id="FormUpdate" onclose={handleCloseUpdate} />
                )}
